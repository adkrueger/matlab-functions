...
% 
%   Outputs are: 
%   RC         :a numeric array of 1xN steady climb rate in m/s
%   gammamaxRC :a numeric array of 1xN climb angle in radians
%   maxRC      :a numeric array of 1xN maximum steady climb rate in m/s
%   vmaxRC     :a numeric array of 1xN velocity at maximum steady climb 
%               rate in m/s
%   gammamaxRC :a numeric array of 1xN climb angle at maximum steady climb
%               rate in radians
%   
...
%   the presumption that the arguments will be ignored in the output call.
    ...
    switch nargin
        ...
        case 3
            ...
            RC = v.*(TtoW-((Q.*Cd_0)./(W./S_w))-(W./S_w).*(K./Q));
            gamma = asin(RC./v);
    end
    ...
    maxRC = sqrt(((W./S_w).*Z)/(3.*rho.*Cd_0)).*(TtoW.^(3./2)).*(1-(Z./6)-(3./(2.*(TtoW.^2).*(maxLtoD.^2).*Z)));
    vmaxRC = sqrt((TtoW.*(W./S_w).*Z)./(3.*rho.*Cd_0));
